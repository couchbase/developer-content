...
The first step is to add the following block under the tag called *project* of your *pom.xml* file:
...
image::quickstart-java27-cb65/Intellij-run-ok.png[Project Started successfully,720,align=left]
...
== Step 4: Create and query a new document
...
Now, let's create the repository responsible for managing our *User* entity. Create a new interface named *UserRepository* extending the *CouchbaseRepository* class:
...
Note that we decorate our class with the *@Repository* annotation.
This indicates that our class provides the mechanism for storage,
retrieval, search, update, and delete operation on objects.

We also added in the interface the method *findByEmailLike*. This method is named that way because it is using the link:https://docs.spring.io/spring-data/couchbase/docs/current/reference/html/#couchbase.repository.n1ql[Spring Data DSL]. Whenever you use this DSL, Spring automatically generates an implementation for you.
It is worth highlighting that methods like *save*, *saveAll*, *delete*, *deleteById*, *findById*, etc. are also automatically generated when you extend the *CouchbaseRepository* class.
 ...
 * the application starts up. So the method *run* is called as soon as
...
image::quickstart-java27-cb65/Intellij-application-run-succesfully.png[Application Ran Successfully,720,align=left]


TIP: If you forget to create the required indexes, your application will fail to execute the *CmdRunner* class. Check out this xref:quickstart-docker-image-manual-cb65.adoc[tutorial] or run `+create primary index on default+`on link:https://docs.couchbase.com/server/current/tools/query-workbench.html[Couchbase's Query Workbench]. I'm assuming that *default* is the name of your bucket.


In the code above, we first create a new instance of the *User* entity and store them in the database using the *save* method. This is what all three documents will look like on Couchbase:

image::quickstart-java27-cb65/inserted-documents.png[Document Inserted,720,align=left]

In the *CmdRunner* class we also get the document back using its id `user::0001` through the method *findById*.
Finally, we query all user documents where `email like "%@acme.com"` using the method *findByEmailLike*.

Note that the *id* of the document is not stored directly in the body of the JSON, but in a meta space called *metadata*:

image::quickstart-java27-cb65/document-metadata.png[Document Metadata, 360,align=left]
